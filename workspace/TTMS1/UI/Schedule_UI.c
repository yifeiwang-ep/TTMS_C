	...
	if(Schedule_Perst_SelectByID(id, &data)){
		...
		while(1){ 
				...
				printf("\n\n\n\t		=================================================================");
				 ...
				 "放映日期","放映时间","座位数");
				printf("\n\t		-----------------------------------------------------------------");
				printf("\n\t\t\t%-3d 『%d』%-10s『%d』%-10s ", data.id,data.play_id, data.play_name,data.studio_id, data.studio_name);
	...
	if(Schedule_Srv_FetchByID(id, &data)){
		...
		while(1){ 
				...
				printf("\n\t		=================================================================");
				 ...
				 "上映日期","上映时间","票价");
				printf("\ni\t		-----------------------------------------------------------------");
					printf("%-3d 『%d』%-10s『%d』%-10s", data.id, data.play_id,data.play_name,data.studio_id,data.studio_name);
					printf(" %4d-%02d-%02d",data.date.year,data.date.month, data.date.day);
					printf(" %4d:%02d:%02d", data.time.hour, data.time.minute, data.time.second);
...
void Schedule_UI_MgtEntry(int play_id,int flag){
     int i, id;
	char choice;
...
//	Play_Srv_FetchByID(play_id, &pdata);
	...
	do {
		...
		printf("\n\n\n\t		=================================================================");
        	printf("\n\t		**************************剧目%d的演出计划*************************",play_id);
          	printf("\n\t		=================================================================");
		printf("\n\t\t\t %-5s %-15d\t%-12s  %-12s   %-10s\n", "ID","演出厅ID","上映日期","上映时间","座位数");
		printf("\n\t		-----------------------------------------------------------------");
		//显示数据
		for (i = 0, p = (schedule_node_t *) (paging.curPos);
				p != head && i < paging.pageSize; i++){
			printf("\n\t\t\t %-5d\t%5d\t", p->data.id, p->data.studio_id);
			printf("%4d-%02d-%02d\t",p->data.date.year, p->data.date.month, p->data.date.day);
			printf("%2d:%02d:%02d\t", p->data.time.hour, p->data.time.minute, p->data.time.second);
                    printf("  %-5d\n", p->data.seat_count);
			p=p->next;
		...
		printf("\n\t		==================================================================");
        	printf("\n\t		|----------总数%d--------------------------%d/%d页-----------------|",paging.totalRecords, Pageing_CurPage(paging),Pageing_TotalPages(paging));
		printf("\n\t		******************************************************************");
        	printf("\n\t		|      [P]revPage           |              [N]extPage            |");
